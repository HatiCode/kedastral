...
You should see:
- `test-app-*` pods (2 initially, will scale)
...
kubectl set env deployment/load-generator PATTERN=hourly-spike -n test-app
...
# Constant baseline
kubectl set env deployment/load-generator PATTERN=constant -n test-app
```
...
kubectl edit deployment kedastral-forecaster -n test-app
...
Try query: `sum(rate(api_requests_total{namespace="test-app"}[1m]))`
...
kubectl logs -l app=kedastral-scaler -n test-app | tail -20

# Check forecast age (should be < 60s)
kubectl logs -l app=kedastral-forecaster -n test-app | grep "forecast_age"
```

### Forecast is stale?
```bash
# Check Prometheus connectivity
kubectl logs -l app=kedastral-forecaster -n test-app | grep -i error

# Verify Prometheus has data
kubectl port-forward -n monitoring svc/prometheus-operated 9090:9090
# Open UI and run: api_requests_total{namespace="test-app"}
```
